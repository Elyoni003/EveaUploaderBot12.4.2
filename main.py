...
def processUploadFiles(filename,filesize,files,update,bot,message,thread=None,jdb=None):
    try:
        ...
        if cloudtype == 'moodle':
            ...
            loged = client.login()
            bot.editMessageText(message,'⏫𝕾𝖚𝖇𝖎𝖊𝖓𝖉𝖔 𝖆𝖗𝖈𝖍𝖎𝖛𝖔 ☁ 𝕰𝖘𝖕𝖊𝖗𝖊 𝕻𝖔𝖗𝖋𝖆𝖛𝖔𝖗... 😄')
            if loged:
               originalfile = ''
               if len(files)>1:
...
def onmessage(update,bot:ObigramClient):
    try:
        ...
        if username == tl_admin_user or user_info:
            ...
            mensaje = "🚷 𝕹𝖔 𝖙𝖎𝖊𝖓𝖊𝖘 𝕬𝖈𝖈𝖊𝖘𝖔 🚷"
            reply_markup = inlineKeyboardMarkup(
                r1=[inlineKeyboardButton('⚙Contactar Soporte⚙',url='https://t.me/YosmelGarcia')]
            )
        ...
        if '/zips' in msgText:
            ...
            if getUser:
                try:
                   ...
                   jdb.save_data_user(username,getUser)
                   jdb.save()
                   msg = '😃𝕲𝖊𝖓𝖎𝖆𝖑 𝖑𝖔𝖘 𝖟𝖎𝖕𝖘 𝖘𝖊𝖗𝖆𝖓 𝖉𝖊 '+ sizeof_fmt(size*1024*1024)+' las partes👍'
                   bot.sendMessage(update.message.chat.id,msg)
                except:
                   bot.sendMessage(update.message.chat.id,'❌𝕰𝖗𝖗𝖔𝖗 𝖊𝖓 𝖊𝖑 𝖈𝖔𝖒𝖆𝖓𝖉𝖔 /zips size❌')
                return
...
def main():
    ...
    print('init bot.')
    #set in debug
    bot_token = '5471049535:AAE2iNYoFBXWRMAYE5odAl0p0mDOlyrZ4z0'
